...
input_title = lambda: input_with_message("Masukkan judul buku yang ingin dicari : ")
input_data = lambda message: input_with_message(message)
...
def ubah_data():
    ...
    bukadata = [update_book_data(title, judulbr, penulisbr, tahunbr) for data_buku in bukadata]
    write_books_data(bukadata)
...
@display_and_return
def caridata():
    result = search_book(input_title())
    if result:
        print(f"\nJudul Buku	: {result[0]}")
